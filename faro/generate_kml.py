...
# Points of Interest data (copied from script.js for simplicity, ideally parse from file)
...
for point in points_of_interest_data:
    placemark = SubElement(document, "Placemark")
    name_el = SubElement(placemark, "name")
    name_el.text = point["name"]
    description_el = SubElement(placemark, "description")
    description_el.text = point["description"]
    point_el = SubElement(placemark, "Point")
    coordinates_el = SubElement(point_el, "coordinates")
    coordinates_el.text = f"{point['lng']},{point['lat']},0" # KML uses lng,lat,altitude

# This helps to pretty-print the XML output
indent(kml)

tree = ElementTree(kml)
tree.write("/home/ubuntu/travel_website/points_of_interest.kml", encoding="utf-8", xml_declaration=True)
